  ...
  const menuMessage = await bot.sendMessage(chatId, menuText, {
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: '🏗 У меня объект / ищу людей', callback_data: 'add_order' }],
...
📢 Если захочешь разместить объявление в закрытых ветках — просто напиши админу: @wkmnnn.`;
    ...
    await bot.sendMessage(chatId, successText, {
      reply_markup: {
        inline_keyboard: [
          [{ text: '🔁 Заполнить ещё одну анкету', callback_data: 'add_contractor' }],
...
📢 Чтобы ускорить публикацию объявления в ветке — напиши админу: @wkmnnn.`;
    ...
    await bot.sendMessage(chatId, successText, {
      reply_markup: {
        inline_keyboard: [
          [{ text: '➕ Добавить ещё один запрос', callback_data: 'add_order' }],
          [{ text: '🧱 Заполнить анкету исполнителя', callback_data: 'add_contractor' }],
          [{ text: '🏠 В главное меню', callback_data: 'main_menu' }]
        ]
      }
    });
...
async function finishSupplierForm(chatId, userId) {
  ...
  if (result.success) {
    const successText = `Готово ✅
Ты добавлен(а) в базу поставщиков / техники «Голоса Стройки».

💡 Это значит, что:
— тебя можно будет быстро находить по запросу материалов / техники;
— дальше бот и админы смогут подбирать тебя под запросы участников.

📢 Если хочешь оформить отдельный пост о своих услугах — напиши админу: @wkmnnn.`;

    await bot.sendMessage(chatId, successText, {
      reply_markup: {
        inline_keyboard: [
          [{ text: '➕ Добавить ещё одну позицию', callback_data: 'add_supplier' }],
          [{ text: '🏠 В главное меню', callback_data: 'main_menu' }]
        ]
      }
    });
